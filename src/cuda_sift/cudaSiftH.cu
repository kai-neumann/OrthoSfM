...
void ExtractSift(SiftData &siftData, CudaImage &img, int numOctaves, double initBlur, float thresh, float lowestScale, bool scaleUp, float *tempMemory) 
{
  ...
  if (!scaleUp) {
    ...
    LowPass(lowImg, img, max((float)initBlur, 0.001f));
    TimerGPU timer1(0);
...
/*
 * =====================================================================================================================
 * ADDED BY KAI NEUMANN
 * =====================================================================================================================
 */
void CopySiftData(const SiftData &src, SiftData &dst, bool host, bool dev)
{
  // Set point number
  ...
  // Copy to host data
  if(host) {
    memcpy(dst.h_data, src.h_data, src.numPts * sizeof(SiftPoint));
  }

  // Also copy device data
  if(dev) {
    safeCall(cudaMemcpy(dst.d_data, src.d_data, src.numPts * sizeof(SiftPoint), cudaMemcpyDeviceToDevice));
  }
}
/*
 * =====================================================================================================================
